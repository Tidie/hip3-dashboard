...
async function fetchAllHip3Fills(address, maxFills = MAX_FILLS_TO_FETCH) {
    ...
    let iterations = 0;
    const maxIterations = Math.ceil(maxFills / BATCH_SIZE) + 5; // Safety limit
    
    console.log(`[${address}] Starting to fetch fills...`);
    
    while (iterations < maxIterations) {
        iterations++;
        
        try {
            // Use userFillsByTime for pagination
            ...
            if (fills.length === 0) {
                console.log(`[${address}] No more fills found. Total iterations: ${iterations}`);
                break;
            }
            
            // Filter HIP-3 fills only (coin contains ':')
            const hip3Fills = fills.filter(f => f.coin && f.coin.includes(':'));
            allHip3Fills = [...allHip3Fills, ...hip3Fills];
            
            console.log(`[${address}] Iteration ${iterations}: Got ${fills.length} fills (${hip3Fills.length} HIP-3), total HIP-3: ${allHip3Fills.length}`);
            
            // If we got less than BATCH_SIZE, we've reached the end
            if (fills.length < BATCH_SIZE) {
                console.log(`[${address}] Reached end of fills (got ${fills.length} < ${BATCH_SIZE})`);
                break;
            }
            
            // If we've hit our limit, stop
            if (allHip3Fills.length >= maxFills) {
                console.log(`[${address}] Reached max fills limit: ${maxFills}`);
                break;
            }
            
            // Get oldest timestamp for next iteration
            const oldestTime = Math.min(...fills.map(f => f.time));
            endTime = oldestTime - 1; // Go back further in time
            
            // Small delay to avoid rate limiting
            await new Promise(r => setTimeout(r, 100));
            
        } catch (e) {
            console.error(`[${address}] Error fetching fills:`, e.message);
            break;
        }
    }
    
    console.log(`[${address}] Finished fetching. Total HIP-3 fills: ${allHip3Fills.length}`);
    return allHip3Fills;
}
