...
const LARGE_ACCOUNT_THRESHOLD = 15000; // 15k trades - API limit
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (event.httpMethod === 'GET' && (fullPath.endsWith('/leaderboard') || fullPath.includes('/leaderboard/'))) {
            ...
            // Parse with explicit defaults to avoid NaN
            ...
            console.log(`[LEADERBOARD] page=${page}, limit=${limit}, offset=${offset}`);
                ...
                return {
                    ...
                    body: JSON.stringify({
                        ...
                        data: {
                            ...
                            markets: null,
                            pnl_history: null,
                            message: `This account has ${existingStats.trades_count.toLocaleString()} trades. Detailed breakdown is not available for accounts with 15k+ trades due to API limitations.`
                        }
                    })
                };
            }
