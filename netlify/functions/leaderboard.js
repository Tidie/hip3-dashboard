...
// Helper: fetch ALL HIP-3 fills for a user (paginated - up to 50 calls for Netlify timeout)
async function fetchAllHip3Fills(address) {
    ...
    if (fills.length >= 2000) {
        ...
        while (totalCalls < MAX_CALLS) {
            try {
                ...
                totalRawFills += olderFills.length;
                
                if (olderFills.length === 0) break;
                ...
                if (olderFills.length < 2000) break; // No more fills
    ...
    console.log(`Fetched ${allFills.length} HIP-3 fills, ${totalRawFills} total raw fills`);
    return { fills: allFills, totalRawFills, hitLimit };
}
...
// Save individual fills to hip3_fills table (with deduplication via tid)
async function saveFills(address, fills) {
    if (fills.length === 0) return { newFills: 0, totalStored: 0 };
    
    try {
        // Get existing tids for this address
        const existing = await axios.get(
            `${SUPABASE_URL}/rest/v1/hip3_fills?address=eq.${address.toLowerCase()}&select=tid`,
            { headers: supabaseHeaders() }
        );
        const existingTids = new Set((existing.data || []).map(f => f.tid));
        
        // Filter out already saved fills
        const newFills = fills.filter(f => f.tid && !existingTids.has(f.tid));
        
        if (newFills.length === 0) {
            return { newFills: 0, totalStored: existingTids.size };
        }
        
        // Prepare fills for insertion
        const fillsData = newFills.map(f => ({
            tid: f.tid,
            oid: f.oid,
        ...
        let inserted = 0;
        for (let i = 0; i < fillsData.length; i += 500) {
            const batch = fillsData.slice(i, i + 500);
            await axios.post(
        ...
        console.log(`Saved ${inserted} new fills for ${address}`);
        return { newFills: inserted, totalStored: existingTids.size + inserted };
    } catch (e) {
        console.log('saveFills error:', e.message);
        return { newFills: 0, totalStored: 0 };
    }
}
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (event.httpMethod === 'GET' && (fullPath.endsWith('/leaderboard') || fullPath === '/.netlify/functions/leaderboard')) {
            ...
            const sortBy = params.sort || params.sortBy || 'total_volume';
        ...
        if (event.httpMethod === 'POST' && fullPath.endsWith('/claim')) {
            ...
            const result = await fetchAllHip3Fills(address);
            let hip3Fills = result.fills;
            let hitLimit = result.hitLimit;
            let totalRawFills = result.totalRawFills;

            if (hip3Fills.length === 0) {
                // Also try with original case (some APIs are case-sensitive)
                if (rawAddress !== address) {
                    const retryResult = await fetchAllHip3Fills(rawAddress);
                    hip3Fills = retryResult.fills;
                    hitLimit = retryResult.hitLimit;
                    totalRawFills = retryResult.totalRawFills;
                }
                ...
                if (hip3Fills.length === 0) {
                    return {
                        ...
                        body: JSON.stringify({
                            success: false,
                            message: `No HIP-3 trades found for this address. Make sure you have traded on HIP-3 markets (tokens with ":" in the name like xyz:TOKEN). Checked up to 100,000 most recent trades.`
                        })
                    };
                }
            }

            // Save individual fills to hip3_fills table for complete history
            const saveResult = await saveFills(address, hip3Fills);
            console.log(`Saved fills result: ${saveResult.newFills} new, ${saveResult.totalStored} total`);

            // Get ALL fills from DB (complete history, not just what API returned)
            let allFillsFromDB = hip3Fills; // fallback to API fills
            try {
                const dbFillsRes = await axios.get(
                    `${SUPABASE_URL}/rest/v1/hip3_fills?address=eq.${address}&select=coin,px,sz,fee,closed_pnl,trade_time`,
                    { headers: supabaseHeaders() }
                );
                if (dbFillsRes.data && dbFillsRes.data.length > 0) {
                    // Convert DB format to computeStats format
                    allFillsFromDB = dbFillsRes.data.map(f => ({
                        coin: f.coin,
                        px: f.px,
                        sz: f.sz,
                        fee: f.fee,
                    ...
                    console.log(`Using ${allFillsFromDB.length} fills from DB (complete history)`);
                }
            } catch (e) {
                console.log('Could not fetch from DB, using API fills:', e.message);
            }

            // Compute stats from COMPLETE history
            const stats = computeStats(allFillsFromDB);
            ...
            return {
                ...
                body: JSON.stringify({
                    ...
                    data: {
                        ...
                        total_raw_fills: totalRawFills,
                        hit_api_limit: hitLimit,
                        api_limit_note: hitLimit ? 'This address has more than 100k trades. Stats are based on available data. Full history is being indexed by our bot.' : null
                    }
                })
            };
        }
        ...
        // ──────────────────────────────────────────────
        // GET /trader?address= - Get trader profile for modal
        // ──────────────────────────────────────────────
        if (event.httpMethod === 'GET' && fullPath.includes('/trader')) {
            const params = event.queryStringParameters || {};
            ...
            if (response.data.length === 0) {
                return {
                    ...
                    body: JSON.stringify({
                        success: false,
                        message: 'Trader not found. They need to claim their position first.'
                    })
                };
            }
