...
// Helper: fetch ALL HIP-3 fills for a user (paginated up to 10k)
async function fetchAllHip3Fills(address) {
    ...
    // Step 2: If we got 2000 fills, paginate backwards (up to 5 total calls = 10k max)
    if (fills.length >= 2000) {
        ...
        while (totalCalls < 5) {
            try {
                ...
                totalCalls++;
                console.log(`Pagination ${totalCalls}/5: +${olderHip3.length} HIP-3 fills (total: ${allFills.length})`);
