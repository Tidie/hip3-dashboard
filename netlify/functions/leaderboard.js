...
exports.handler = async (event) => {
    ...
    try {
        // ──────────────────────────────────────────────
        // GET /leaderboard - Get top traders (with sorting & time filter)
        // ──────────────────────────────────────────────
        if (event.httpMethod === 'GET' && path === '') {
            const limit = parseInt(event.queryStringParameters?.limit || '50');
            const sortBy = event.queryStringParameters?.sortBy || 'total_volume';
            ...
            // For all-time, use database directly
            if (period === 'all') {
            ...
            // For time-filtered periods, fetch top addresses and recalculate
            const periodHours = { '24h': 24, '7d': 168, '30d': 720 };
            const hours = periodHours[period] || 168; // Default 7d
            const cutoffTime = Date.now() - (hours * 60 * 60 * 1000);
            
            // Get top 100 addresses from DB as candidates
            const candidatesRes = await axios.get(
                `${SUPABASE_URL}/rest/v1/hip3_traders?select=address&order=total_volume.desc&limit=100`,
                { headers: supabaseHeaders() }
            );
            
            const addresses = candidatesRes.data.map(t => t.address);
            const tradersWithStats = [];
            
            // Fetch fills for each address and filter by time
            for (const address of addresses.slice(0, 50)) { // Limit to 50 for performance
                try {
                    ...
                    // Filter HIP-3 fills within time window
                    const filteredFills = allFills.filter(f => 
                        f.coin && f.coin.includes(':') && f.time >= cutoffTime
                    );
                    
                    if (filteredFills.length > 0) {
                        const stats = computeStats(filteredFills);
                        tradersWithStats.push({
                            address,
                            ...
                            roi: stats.total_volume > 0 ? (stats.total_pnl / stats.total_volume * 100) : 0
                        });
                    }
                } catch (e) {
                    // Skip failed fetches
            ...
            tradersWithStats.sort((a, b) => b[sortField] - a[sortField]);
            ...
            return {
                ...
                body: JSON.stringify({
                    success: true,
                    period: period,
                    data: tradersWithStats.slice(0, limit)
                })
            };
        }
        ...
        if (event.httpMethod === 'GET' && path.startsWith('/address/')) {
            const rawAddress = path.replace('/address/', '');
            const address = rawAddress.toLowerCase();
