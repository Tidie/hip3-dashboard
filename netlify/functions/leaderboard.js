...
exports.handler = async (event) => {
    ...
    try {
        ...
        // Uses data from leaderboard_stats (populated by S3 bot) as primary source
        // Falls back to API only if address not found in database
        // ──────────────────────────────────────────────
        if (event.httpMethod === 'POST' && fullPath.endsWith('/claim')) {
            ...
            if (existingStats.data && existingStats.data.length > 0) {
                // Use existing data from database (more complete from S3 sync)
                const dbData = existingStats.data[0];
                fromDatabase = true;
                
                stats = {
                    total_volume: dbData.total_volume || 0,
                    total_fees: dbData.total_fees || 0,
                    total_pnl: dbData.total_pnl || 0,
                    trades_count: dbData.trades_count || 0,
                    pairs_traded: dbData.pairs_traded || 0,
                    markets: {},
                    pnl_history: [],
                    score: (dbData.total_volume || 0) * 0.5 + (dbData.total_fees || 0) * 200
                };
                
                // Try to get detailed markets from hip3_traders if available
                const traderDetails = await axios.get(
                    `${SUPABASE_URL}/rest/v1/hip3_traders?address=eq.${address}`,
                    { headers: supabaseHeaders() }
                );
                
                if (traderDetails.data && traderDetails.data.length > 0) {
                    stats.markets = traderDetails.data[0].markets || {};
                    stats.pairs_traded = Object.keys(stats.markets).length || stats.pairs_traded;
                }
            } else {
                // Fallback: Fetch from Hyperliquid API if not in database
                ...
                // Compute stats from API fills
                stats = computeStats(hip3Fills);
            ...
            // Prepare trader data for hip3_traders table
            const traderData = {
                address: address,
                total_volume: stats.total_volume,
                total_fees: stats.total_fees,
            ...
            // Upsert to hip3_traders (for detailed view)
            ...
            if (existingTrader.data.length > 0) {
                // Only update if we have more data (don't overwrite with less)
                if (!fromDatabase || Object.keys(stats.markets).length > 0) {
                    await axios.patch(
                        `${SUPABASE_URL}/rest/v1/hip3_traders?address=eq.${address}`,
                        traderData,
                        { headers: supabaseHeaders() }
            ...
            return {
                ...
                body: JSON.stringify({
                    ...
                    data: {
                        ...
                        fills_analyzed: stats.trades_count,
                        data_source: fromDatabase ? 'database (S3 sync)' : 'hyperliquid API'
                    }
                })
            };
        }
