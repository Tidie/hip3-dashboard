...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (event.httpMethod === 'POST' && fullPath.endsWith('/claim')) {
            ...
            // Calculate stats by deployer from markets
            const markets = dbData.markets || {};
            const deployerStats = {};
            for (const [coin, stats] of Object.entries(markets)) {
                const deployer = coin.split(':')[0];
                if (!deployerStats[deployer]) {
                    deployerStats[deployer] = { volume: 0, fees: 0, pnl: 0, trades: 0 };
                }
                deployerStats[deployer].volume += stats.volume || 0;
                deployerStats[deployer].fees += stats.fees || 0;
                deployerStats[deployer].pnl += stats.pnl || 0;
                deployerStats[deployer].trades += stats.trades || 0;
            }
