...
// Helper: fetch ALL HIP-3 fills for a user
...
exports.handler = async (event) => {
    ...
    try {
        // ──────────────────────────────────────────────
        // GET /leaderboard - Get top traders (with sorting + pagination)
        // ──────────────────────────────────────────────
        if (event.httpMethod === 'GET' && (fullPath.endsWith('/leaderboard') || fullPath === '/.netlify/functions/leaderboard')) {
            const params = event.queryStringParameters || {};
            const page = Math.max(1, Math.min(10, parseInt(params.page || '1'))); // Max 10 pages
            const limit = 100; // Fixed at 100 per page
            ...
            const response = await axios.get(
                `${SUPABASE_URL}/rest/v1/hip3_traders?select=*&order=${sortField}.desc&limit=${limit}&offset=${offset}`,
                { 
                    headers: { 
                        ...supabaseHeaders(), 
                        'Prefer': 'count=exact'
            ...
            const totalPages = Math.min(10, Math.ceil(totalCount / limit)); // Cap at 10 pages (1000 traders)
            ...
            return {
                ...
                body: JSON.stringify({
                    ...
                    pagination: {
                        ...
                        limit: limit,
                        offset: offset,
                        total: Math.min(totalCount, 1000), // Cap display at 1000
