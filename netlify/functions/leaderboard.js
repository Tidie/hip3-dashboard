...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (event.httpMethod === 'GET' && (fullPath.endsWith('/leaderboard') || fullPath === '/.netlify/functions/leaderboard')) {
            ...
            const allowedSorts = ['total_volume', 'total_pnl', 'total_fees', 'trades_count'];
            const sortField = allowedSorts.includes(sortBy) ? sortBy : 'total_volume';

            const response = await axios.get(
                `${SUPABASE_URL}/rest/v1/leaderboard_stats?select=*&order=${sortField}.desc&limit=${limit}`,
        ...
        if (event.httpMethod === 'POST' && fullPath.endsWith('/claim')) {
            ...
            // Also upsert into leaderboard_stats for the leaderboard display
            await axios.post(
            ...
            // Get rank by counting traders with higher volume (from leaderboard_stats)
            ...
            // Get total traders count (from leaderboard_stats)
